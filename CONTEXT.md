...
A Day Record becomes a frozen snapshot only once it is **touched** (a Block ticked done, an
ad-hoc Block added, or any tracked time). While a present-or-future day is still **pristine**
(untouched) it keeps mirroring the live Template, re-seeding when the Template changes, so a
day auto-created before the routine was finished still fills in. Pure day-only reordering is
preserved and does not count as touching. Past days never re-sync.
